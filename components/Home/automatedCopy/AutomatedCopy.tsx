...
const questions = [
  ...
  {
    questionText:
      "In which country did the legend of the Headless Horseman originate?",
    answerOptions: [
  ...
  {
    questionText:
      "What is the traditional name for a Japanese ghost or spirit?",
    answerOptions: [
  ...
  {
    questionText:
      "Which legendary monster is known to suck the blood of livestock?",
    answerOptions: [
...
const AutomatedCopy: React.FC<{
  setSurveyComplete: SetSurveyCompleteFunction;
}> = ({ setSurveyComplete }) => {
  ...
  const { typedText, completed } = useTypeAnimation(
    activeStep < questions.length ? questions[activeStep].questionText : "", // stop the typing animation after the last question
    80
  );
        ...
        <CardContent>
          {isLastQuestionAnswered ? (
            ...
            <FormControl component="fieldset">
              <Typography variant="h4" fontWeight="bold" component="legend">
                {typedText}
              </Typography>
              {completed && (
                <RadioGroup
